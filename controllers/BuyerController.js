class BuyerController {
  ...
  async viewCart(req, res,next) {
      // Logic for viewing the cart (buyer)
      try {
        const connection = req.db;
  ...
  async removeCartItem(req, res,next) {
      // Logic for removing an item from the cart (buyer)
      try {
        const connection = req.db;
  ...
  async viewAllOrders(req, res,next) {
      // Logic for viewing all orders by a buyer
      try {
        const connection = req.db;
        const [details] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id=(Select buyer_id from buyer_accounts where status=1)`);
  ...
  async viewOrderDetails(req, res,next) {
      // Logic for viewing order details (buyer)
      try {
        const connection = req.db;
        const order_id = req.params.order_id;
        const [details] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id=(Select buyer_id from buyer_accounts where status=1)`);
        const query = `SELECT oi.order_id, oi.order_status, oi.product_id, oi.quantity, p.product_name, p.price, oi.item_id, o.payment_choice
            FROM order_items oi
  ...
  async dashboardSettings(req, res,next) {
      // Logic for buyer dashboard settings
      try {
        const connection = req.db;
  ...
  async buyerDetails(req, res,next) {
      // Logic for buyer details page (buyer module)
      try {
        const connection=req.db;
  ...
  async tasksGridLayout(req, res,next) {
    // Logic for products grid layout page
    try {
        const connection = req.db;
        const [details] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id=(Select buyer_id from buyer_accounts where status=1)`);
  ...
  async openProductPage(req, res,next) {
      // Logic for opening the product page (buyer account)
      try {
        const connection=req.db;
        const [details] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id=(Select buyer_id from buyer_accounts where status=1)`)
  ...
  async browseCompanyFreelancer(req, res,next) {
      // Logic for browsing company freelancers
      try {
        const connection = req.db;
        const [details] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id=(Select buyer_id from buyer_accounts where status=1)`)
  ...
  async viewSingleCompanyPageFreelancer(req, res,next) {
      // Logic for viewing a single company page (buyer)
      try {
        const connection = req.db;
  ...
  async viewBuyerReviews(req, res,next) {
      // Logic for viewing buyer's reviews
      try {
        const connection = req.db;
        const [details] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id=(Select buyer_id from buyer_accounts where status=1)`)
  ...
  async viewBuyerProfile(req, res,next) {
      // Logic for viewing buyer's own profile
      try {
        const connection = req.db;
  ...
  async setOrderReceived(req, res,next) {
      // Logic for setting order status to received (buyer)
      try {
        const connection = req.db;
  ...
  async markAsDelivered(req, res,next) {
      // Logic for marking a product as delivered by the buyer
      try {
        const item_id = req.params.item_id
        await connection.execute(`UPDATE order_items SET order_status="Delivered" WHERE item_id=${item_id}`)
  ...
  async confirmOrder(req, res,next) {
      // Logic for confirming an order from cart by the buyer
      try {
        const connection = req.db;
  ...
  async updateBuyerDetails(req, res,next) {
      ...
      try {
        const connection = req.db;
        // Rest of the code for updateDashboardSettings route...
