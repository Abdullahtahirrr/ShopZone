...
class BuyerController {
  async browseCompanies(req, res, next) {
    try {
      ...
      if (req.session) {
        ...
        const [buyerDetails] = await connection.execute(`SELECT * FROM buyer_profile WHERE buyer_id = ?`, [userId]);
  
        if (buyerDetails.length > 0) {
          details = buyerDetails[0];
        } else {
          const sellerId = req.session.user.seller_id;

          const [sellerDetails] = await connection.execute(`SELECT * FROM seller_accounts WHERE seller_id = ?`, [sellerId]);
          details = sellerDetails[0];
        }
      }
  ...
  async viewCart(req, res, next) {
    try {
      ...
      res.render('dashboard-my-active-bids.ejs', { details: details[0], cartItems: cartItems });
    } catch (error) {
      next(error);
    }
  }
  ...
  async viewOrderDetails(req, res, next) {
    try {
      ...
      const query = `
        ...
        WHERE oi.order_id = ? AND oi.buyer_id = ?
      `;
  ...
  async addBuyerDetails(req, res,next) {
    // Logic for updating buyer details
      ...
      try {
        ...
        const [result] = await connection.execute(query, [buyerId,fname, lname, email, picpath]);
